...
public class Item : MonoBehaviour
{
    ...
    {
        if (evolution.envolutionsLevel > currentLevel + levelUpAmount)
        {
            Debug.LogWarning(string.Format("Evolution failed. Current level {0}, evolutiion level {1}", currentLevel, evolution.envolutionsLevel));
            return false;
        }
        ...
        foreach (ItemData.Evolution.Config c in evolution.catalysts)
        {
            ...
            {
                Debug.LogWarning(string.Format("Evolution failed. Missing item {0}", c.itemType.name));
                return false;
            }
        }
    ...
    //What effect you receive when you equip the item
    public virtual void OnEquip()
    {
        //Weapon Equip logic will go here later
    }

    public virtual void OnUnequip()
    {
        //Weapon Unequip logic will go here later
    }
}
